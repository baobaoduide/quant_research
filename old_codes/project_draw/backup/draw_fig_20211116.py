...
def draw_stacked_bar(bond_data, fig_size=(9, 6), title_name='', ylabel='规模(亿元)', xlabel='时间', columns_use=['国债', '国开债'], col_colors=['群青', '中欧橙']):
    ...
    ax.set_ylabel(ylabel, color=ax_color)
    ax.set_xlabel(xlabel, color=ax_color)
    xlab = pd.Series(bond_data_use.index).dt.strftime('%Y/%m')
...
def draw_grouped_bar(bond_data, fig_size=(9, 6), title_name='', ylabel='规模(亿元)', xlabel='时间', columns_use=['国债', '国开债', '进出口债', '农发债'], col_colors=['群青', '浅蓝', '中欧橙', '50% 灰']):
    ...
    bwidth = 0.15
    ax_color = set_color('80% 灰')
    ax = bond_data_use.plot.bar(figsize=fig_size)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['bottom'].set_color(set_color('30% 灰'))
    ax.spines['left'].set_color(ax_color)
    ax.bar(x - bwidth * 3/2, bond_data_use['国债'], width=bwidth, label='国债', color=set_color('群青'))
    ax.bar(x - bwidth * 1/2, bond_data['国开债'], width=bwidth, label='国开债', color=set_color('浅蓝'))
    ax.bar(x + bwidth * 1/2, bond_data['进出口债'], width=bwidth, label='进出口债', color=set_color('70% 灰'))
    ax.bar(x + bwidth * 3/2, bond_data['农发债'], width=bwidth, label='农发债', color=set_color('中欧橙'))
    ax.set_ylabel(ylabel, color=ax_color)
    ax.set_xlabel(xlabel, color=ax_color)
    ax.set_xticks(x)
