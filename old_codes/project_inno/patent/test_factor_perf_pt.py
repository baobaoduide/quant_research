...
from old_codes.load_data.load_factor import load_std_factor_pt
...
def test_ic_ir(factor_df_std, ashare_ret, fig_label):
    ...
    plt.legend()
    plt.title('Factor Monthly RankIC '+fig_label)
    path_fig = r'E:\NJU_term4\TF_Intern\project_comp_inno_growth\figure_pt_comp_factor\\'+fig_label+'rank_ic_monthly.png'
    plt.savefig(path_fig)
    plt.close()
    return factor_perf_df


def cal_grouped_excess_ret(factor_df_std, ashare_ret, fig_label):
    factor_and_return = pd.merge(factor_df_std, ashare_ret, on=['CalcDate', 'Code'])
    ...
    plt.legend()
    plt.grid(axis='y', linestyle='-', zorder=1)
    plt.title('Grouped Excess Return '+fig_label)
    path_fig = r'E:\NJU_term4\TF_Intern\project_comp_inno_growth\figure_pt_comp_factor\\' + fig_label + 'grouped_excess_ret.png'
    plt.savefig(path_fig)
    plt.close()
    return grouped_quantile_ret_mean


def draw_nv_curve(factor_df_std, ashare_ret, bench_index_ret, fig_label):
    factor_and_return = pd.merge(factor_df_std, ashare_ret, on=['CalcDate', 'Code'])
    quantile_num = 5
    ...
    ax.set_title('long short net value curve '+fig_label)
    lns = p1 + p2 + p3 + p4+ p5
    labs = [l.get_label() for l in lns]
    ax.legend(lns, labs, loc=0)
    # ax.grid(axis='y', linestyle='-', zorder=1)
    fig.tight_layout()
    path_fig = r'E:\NJU_term4\TF_Intern\project_comp_inno_growth\figure_pt_comp_factor\\' + fig_label + 'nv_curve.png'
    plt.savefig(path_fig)
    plt.close()
    return nv_perform


def test_factor_performance(start_date, end_date, factor_type, industry_citic):
    factor_df_std = load_std_factor_pt(start_date, end_date, factor_type, industry_citic)
    #
    ...
    #
    fig_label = factor_type + '_' + industry_citic
    factor_perf_df = test_ic_ir(factor_df_std, ashare_ret, fig_label)
    ex_ret_grouped = cal_grouped_excess_ret(factor_df_std, ashare_ret, fig_label)
    nv_perform = draw_nv_curve(factor_df_std, ashare_ret, bench_index_ret, fig_label)
    return factor_perf_df, ex_ret_grouped, nv_perform


start_date = '2011-01-01'
end_date = '2020-12-31'
industrys = ['医药', '机械', '电子', '汽车', '通信', '计算机', '基础化工', '电力设备及新能源']
factor_types = ['pt_examing_comp', 'pt_authorized_comp', 'pt_utility_comp', 'pt_authorized_num_all', 'pt_former_quote_all']
for j in range(len(factor_types)):
    ic_ir_list = []
    nv_ret_list = []
    for i in range(len(industrys)):
        factor_type = factor_types[j]
        industry_citic = industrys[i]
        print(j+1, factor_type, i+1, industry_citic)
        factor_perf_df, ex_ret_grouped, nv_perform = test_factor_performance(start_date, end_date, factor_type, industry_citic)
        factor_perf_df['industry'] = industry_citic
        nv_perform['industry'] = industry_citic
        ic_ir_list.append(factor_perf_df)
        nv_ret_list.append(nv_perform)
    ic_ir_df = pd.concat(ic_ir_list, axis=1)
    nv_ret_df = pd.concat(nv_ret_list)
    path_ic_ir_df = os.path.join(os.getcwd(), 'ic_ir_'+factor_type+'.csv')
    ic_ir_df.to_csv(path_ic_ir_df, encoding='utf_8_sig')
    path_nv_ret_df = os.path.join(os.getcwd(), 'ret_result_df_'+factor_type+'.csv')
    nv_ret_df.to_csv(path_nv_ret_df, encoding='utf_8_sig')
