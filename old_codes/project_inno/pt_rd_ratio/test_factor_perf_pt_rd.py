...
from old_codes.load_data.load_factor import load_std_factor_pt_rd
...
def test_ic_ir(factor_df_std, ashare_ret, industry_citic):
    ...
    path_fig = r'E:\NJU_term4\TF_Intern\project_comp_inno_growth\figure_pt_rd_ratio\\'+industry_citic+'rank_ic_monthly.png'
...
def cal_grouped_excess_ret(factor_df_std, ashare_ret, industry_citic):
    ...
    path_fig = r'E:\NJU_term4\TF_Intern\project_comp_inno_growth\figure_pt_rd_ratio\\' + industry_citic + 'grouped_excess_ret.png'
    plt.savefig(path_fig)
    plt.close()
...
def draw_nv_curve(factor_df_std, ashare_ret, bench_index_ret, industry_citic):
    ...
    fig.tight_layout()
    path_fig = r'E:\NJU_term4\TF_Intern\project_comp_inno_growth\figure_pt_rd_ratio\\' + industry_citic + 'nv_curve.png'
    plt.savefig(path_fig)
    plt.close()
    return nv_perform
...
def test_factor_performance(start_date, end_date, industry_citic):
    factor_df_std = load_std_factor_pt_rd(start_date, end_date, industry_citic)
    #
...
for i in range(len(industrys)):
    ...
    ret_nv_list.append(nv_perform)
ic_ir_df = pd.concat(ic_ir_list, axis=1)
ret_nv_df = pd.concat(ret_nv_list)
path_factor_perf_df = os.path.join(os.getcwd(), 'pr_ratio_ic_ir.csv')
ic_ir_df.to_csv(path_factor_perf_df, encoding='utf_8_sig')
path_nv_perform = os.path.join(os.getcwd(), 'pr_ratio_result_df.csv')
ret_nv_df.to_csv(path_nv_perform, encoding='utf_8_sig')
