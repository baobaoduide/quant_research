...
from filter_industry_sample2 import filter_zhbdtxp
...
def prepare_space_space(basic_info, ashare_st, mv_amount, start_date, end_date,
    ...
    code_list = code_all[code_all['date_in'] < date_cut]
    # 确立样本空间
    sample_space = code_indicator[code_indicator['code'].isin(code_list['code'])]
    ...
    # 筛选的第二步。。。
    filter_ind = filter_zhbdtxp('2022-06-30')
    ...
    member_new = sample_space_use_ind[~sample_space_use_ind['code'].isin(member_last)].sort_values(by='mv_mean',
                                                                                                   ascending=False)
    path_data = os.path.join(path_save, '中华半导体芯片_入选空间.csv')
    member_new.to_csv(path_data, encoding='gbk', index=False)
    path_data = os.path.join(path_save, '中华半导体芯片_保留空间.csv')
    ...
    path_data = os.path.join(path_save, '中华半导体芯片_变动.csv')
    member_change.to_csv(path_data, encoding='gbk', index=False)
    path_data = os.path.join(path_save, '中华半导体芯片_指数样本.csv')
...
if __name__ == '__main__':
    predict_index('2022-06-30', '中华半导体芯片')
