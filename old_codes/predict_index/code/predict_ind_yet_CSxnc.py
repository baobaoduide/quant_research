...
from filter_industry_sample2 import filter_CSxnc
...
def predict_index(term, index_name):
    ...
    top50 = sample_space_use_ind.iloc[:change_num]
    #
    member_old = sample_space_use[sample_space_use['code'].isin(member_last)].sort_values(by='mv_mean', ascending=False)
    member_new = sample_space_use_ind[~sample_space_use_ind['code'].isin(member_last)].sort_values(by='mv_mean', ascending=False)
    path_data = os.path.join(path_save, 'CS新能车_入选空间.csv')
    member_new.to_csv(path_data, encoding='gbk', index=False)
    path_data = os.path.join(path_save, 'CS新能车_保留空间.csv')
    ...
    path_data = os.path.join(path_save, 'CS新能车_变动.csv')
    ...
    path_data = os.path.join(path_save, 'CS新能车_指数样本.csv')
    result.to_csv(path_data, encoding='gbk', index=False)
    return result, member_change


if __name__ == '__main__':
    predict_index('2022-06-30', 'CS新能车') # 28个变动也有点多
