...
from filter_industry_sample2 import filter_5Gtx
...
def predict_index(term, index_name):
    ...
    member_new = sample_space_use_ind[~sample_space_use_ind['code'].isin(member_last)].sort_values(by='mv_mean',
                                                                                                   ascending=False)
    path_data = os.path.join(path_save, '5G通信_入选空间.csv')
    member_new.to_csv(path_data, encoding='gbk', index=False)
    path_data = os.path.join(path_save, '5G通信_保留空间.csv')
    ...
    path_data = os.path.join(path_save, '5G通信_变动.csv')
    ...
    path_data = os.path.join(path_save, '5G通信_指数样本选.csv')
    result.to_csv(path_data, encoding='gbk', index=False)
    return result, member_change


if __name__ == '__main__':
    predict_index('2022-06-30', '5G通信') # 前两年剔除20多个的情况也是有的，36个也许也可
