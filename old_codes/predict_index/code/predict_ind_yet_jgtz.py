...
import pandas as pd
import math
from load_data_from_data_base import load_index_list, load_aindexmembers, load_industry_cate_zz, load_cate_info_data
...
def predict_index(term, index_name):
    ...
    board_gn_in = gn_board_info[
        gn_board_info['board_gn'].apply(func=lambda x: '光伏' in x if isinstance(x, str) else False)]
    board_gn_hot_in = gn_board_info[
        gn_board_info['board_gn_hot'].apply(func=lambda x: '光伏' in x if isinstance(x, str) else False)]
    board_industry_in = gn_board_info[
        gn_board_info['board_industry'].apply(func=lambda x: '光伏' in x if isinstance(x, str) else False)]
    # comp_info = gn_board_info[
    #     gn_board_info['comp_brief'].apply(func=lambda x: '光伏' in x if isinstance(x, str) else False)]
    all_codes_boards = pd.concat(
        [board_gn_in['code'], board_gn_hot_in['code'], board_industry_in['code']]).drop_duplicates()
    sample_space_use = sample_space_use[sample_space_use['code'].isin(all_codes_boards['code'])]
    #
    ...
    path_data = os.path.join(path_save, '结构调整_指数样本.csv')
