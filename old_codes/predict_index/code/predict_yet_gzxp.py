...
from filter_industry_sample2 import filter_gzxp
...
def prepare_space_space(basic_info, ashare_st, mv_amount, start_date, end_date,
    ...
    code_indicator = pd.merge(mv_half, amount_year[['code', 'amount_mean']], on=['code'])
    ...
    # 筛选的第二步。。。
    filter_ind = filter_gzxp('2022-06-30')
    ...
    member_new = sample_space_use_ind[~sample_space_use_ind['code'].isin(member_last)].sort_values(by='mv_mean',
                                                                                                   ascending=False)
    path_data = os.path.join(path_save, '国证芯片_入选空间.csv')
    member_new.to_csv(path_data, encoding='gbk', index=False)
    path_data = os.path.join(path_save, '国证芯片_保留空间.csv')
    ...
    #
    old_data = sample_space_use_ind[sample_space_use_ind['code'].isin(member_last)].reset_index(drop=True)
    old_data = old_data.iloc[:24]
    new_data = sample_space_use_ind[~sample_space_use_ind['code'].isin(member_last)].reset_index(drop=True)
    member_in = new_data.iloc[:6].reset_index(drop=True)
    code_out = member_last[~member_last.isin(old_data['code'])]
    ...
    path_data = os.path.join(path_save, '国证芯片_变动.csv')
    member_change.to_csv(path_data, encoding='gbk', index=False)
    result = pd.concat([old_data, member_in]).sort_values(by=['code']).reset_index(drop=True)
