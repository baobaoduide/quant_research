...
from filter_industry_sample2 import filter_zzyl
...
def predict_index(term, index_name):
    ...
    code_not_includ = member_last[~member_last.isin(sample_space_use_ind['code'])]
    member_retain = sample_space_use[sample_space_use['code'].isin(code_not_includ)]
    print('不在筛选行业样本池中需保留的数目为：', len(member_retain))
    #
    ...
    member_new = sample_space_use_ind[~sample_space_use_ind['code'].isin(member_last)].sort_values(by='mv_mean',
                                                                                                   ascending=False)
    path_data = os.path.join(path_save, '中证医疗_入选空间2.csv')
    member_new.to_csv(path_data, encoding='gbk', index=False)
    path_data = os.path.join(path_save, '中证医疗_保留空间.csv')
    member_old.to_csv(path_data, encoding='gbk', index=False)
    top50 = sample_space_use_ind.iloc[:49]
    #
    ...
    path_data = os.path.join(path_save, '中证医疗_变动.csv')
    ...
    path_data = os.path.join(path_save, '中证医疗_指数样本.csv')
    result.to_csv(path_data, encoding='gbk', index=False)
    return result, member_change


if __name__ == '__main__':
    predict_index('2022-06-30', '中证医疗') # 4个变动
