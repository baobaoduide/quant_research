...
if 'macos' in sys_platform:
    folder = r'/Volumes/Intern/data_gk_model/result_202308'
...
def plot_data(name):
    ...
    for idx, name_h in enumerate(names):
        plt.bar(xlabel_, part_df[name_h], bottom=data_stack[idx], color=colors[idx], width=0.3, label=name_h)
    plt.plot(xlabel_, data['实际收益'], color='black', marker='.', linewidth=1, label='实际收益')
    ...
    ax.set_ylabel('百分比 (%)')
    plt.legend()
    path_save_fig = os.path.join(folder, '拆分结果_' + name + '_win.png')
