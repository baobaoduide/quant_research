...
def sum_info2(s_y, e_y):
    ...
    path_ = r'E:\data_gk_model\result_202308\宽基及行业指数汇总结果_'+str(s_y)+'to'+str(e_y)+'.xlsx'
    ...
    plt.scatter(
        xlabel_, sum_df_use["实际收益"], color="black", marker=".", linewidth=1, label="实际收益"
    )
    ax.spines["right"].set_visible(False)
    ax.spines["top"].set_visible(False)
    ax.spines["bottom"].set_color("gray")
    # plt.title(title, fontsize=16)
    plt.xticks(xlabel_, rotation='vertical')
    plt.legend()
    plt.tight_layout()
